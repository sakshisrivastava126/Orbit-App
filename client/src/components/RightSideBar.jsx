...
const RightSideBar = () => {
  ...
  //getting all the images from the messages and set them to state
  useEffect(() => {
    setMsgImages(
    ...
    <div className={`bg-[#8185B2] text-white w-full relative overflow-y-scroll {selectedUser ? "max-md:hidden" : ""}`} >
      <div className='pt-16 flex flex-col items-center gap-2 text-xs font-light mx-auto' >
        ...
        <h1 className='px-10 text-xl font-medium mx-auto flex items-center gap-2' >
          {onlineUser.includes(selectedUser._id) && <p className='flex flex-row w-full h-2' ></p>}
          {selectedUser.fullName}
        </h1>
        <p className='px-10 mx-auto py-5' >{selectedUser.bio}</p>
      </div>
